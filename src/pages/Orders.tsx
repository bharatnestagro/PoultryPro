  ...
  const handleDeliveryPaymentRazorpay = async (order: any) => {
    ...
    try {
      const orderRes = await fetch("/api/create-razorpay-order", {
        ...
        body: JSON.stringify({ amount: order.paymentStatus === 'Paid' ? order.deliveryCharge : (order.totalAmount + order.deliveryCharge) })
      });
  ...
  const handleDeliveryPaymentCashfree = async (order: any) => {
    ...
    try {
      ...
      const response = await fetch("/api/create-cashfree-session", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amount: order.paymentStatus === 'Paid' ? order.deliveryCharge : (order.totalAmount + order.deliveryCharge),
